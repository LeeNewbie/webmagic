...
import us.codecraft.webmagic.pipeline.MongodbPipeline;
...
public class IThomePageProcessor implements PageProcessor {
    ...
    public static final String REG_DETAIL_PAGE_TYPE_ID = "https://www.ithome.com/html/(\\w+)/(\\d+).htm";
    ...
    public void process(Page page) {
        if (REG_HOME_PAGE.equals(page.getUrl().toString())) {
            ...
            for (String link : ls) {
                String id = new PlainText(link).regex(REG_DETAIL_PAGE_TYPE_ID,2).get();
            ...
            if (page.getRequest().getExtra(EXTRA_KEY_ID) == null) {
                id = page.getUrl().regex(REG_DETAIL_PAGE_TYPE_ID,2).get();
            } else {
            ...
            List<String> ls = page.getHtml().links().regex(REG_DETAIL_PAGE).all();
            for (String link : ls) {
                String idElse = new PlainText(link).regex(REG_DETAIL_PAGE_TYPE_ID,2).get();
    ...
    public static void main(String[] args) {
        ...
        Spider.create(new IThomePageProcessor())
                ...
                .addPipeline(new MongodbPipeline("IT3"))
